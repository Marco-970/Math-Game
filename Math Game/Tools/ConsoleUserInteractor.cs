...
");
        ...
        public void DisplayResults(List<(int Points, double Time)> results) => results
            .Select((result, i) => (result, i))
            .ToList()
            .ForEach(result => Console.WriteLine($"| - Game n°{result.i + 1}: {result.result.Points} points. {result.result.Time} seconds. - |"));
